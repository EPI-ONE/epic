...
TEST_F(TestConsensus, ForkedChain) {
    // construct the main chain and fork
    std::deque<ChainStatePtr> dqcs{make_shared_ChainState()};
    ConstBlockPtr forkblk;
    ChainStatePtr split;
...
/*TEST_F(TestConsensus, FindSplittingPoint) {
    ...
    for (int i = 1; i < 5; i++) { // reach height 4
        vMs0.push_back(make_shared_ChainState(fac.CreateConsecutiveRecordPtr(), vMs0[i - 1]));
    }
    auto vMs1 = vMs0;
    ...
    for (int i = 0; i < 6; i++) { // reach height 7
        if (i % 2) {
            vMs0.push_back(make_shared_ChainState(fac.CreateConsecutiveRecordPtr(), vMs0.back(), true));
        } else {
            vMs1.push_back(make_shared_ChainState(fac.CreateConsecutiveRecordPtr(), vMs1.back()));
        }
    }
    ...
    for (int i = 0; i < 2; i++) { // reach height 9
        vMs0.push_back(make_shared_ChainState(fac.CreateConsecutiveRecordPtr(), vMs0.back(), true));
        vMs1.push_back(make_shared_ChainState(fac.CreateConsecutiveRecordPtr(), vMs1.back()));
        vMs2.push_back(make_shared_ChainState(fac.CreateConsecutiveRecordPtr(), vMs2.back()));
        vMs3.push_back(make_shared_ChainState(fac.CreateConsecutiveRecordPtr(), vMs3.back()));
    }
