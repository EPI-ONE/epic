...
bool SigHasLowR(const secp256k1_ecdsa_signature* sig) {
    ...
    // In DER serialization, all values are interpreted as big-endian, signed integers. The highest bit in the integer
    // indicates its signed-ness; 0 is positive, 1 is negative. When the value is interpreted as a negative integer, it
    // must be converted to a positive value by prepending a 0x00 byte so that the highest bit is 0. We can avoid this
    // prepending by ensuring that our highest bit is always 0, and thus we must check that the first byte is less than
    // 0x80.
...
std::optional<CKey> DecodeSecret(const std::string& str) {
    ...
    if (DecodeBase58Check(str, data)) {
        const std::vector<unsigned char> privkey_prefix(1, GetParams().GetKeyPrefix(Params::KeyPrefixType::SECRET_KEY));
...
std::string EncodeSecret(const CKey& key) {
    assert(key.IsValid());
    std::vector<unsigned char> data(1, GetParams().GetKeyPrefix(Params::KeyPrefixType::SECRET_KEY));
