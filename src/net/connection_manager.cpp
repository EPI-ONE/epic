...
static void AcceptCallback(struct evconnlistener *listener, evutil_socket_t fd, struct sockaddr *addr,
    ...
    struct bufferevent *bev = ((ConnectionManager*)ctx)->CreateBufferevent(base, fd, BEV_OPT_CLOSE_ON_FREE|BEV_OPT_THREADSAFE);
...
int ConnectionManager::Listen(uint32_t port, uint32_t local_bind_address) {
    ...
    listener = evconnlistener_new_bind(base, AcceptCallback, this, LEV_OPT_CLOSE_ON_FREE | LEV_OPT_REUSEABLE, -1, (struct sockaddr *) &sock_addr_in, sizeof(sock_addr_in));
