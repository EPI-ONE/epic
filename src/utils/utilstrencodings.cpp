...
bool ParseUInt32(const std::string& str, uint32_t* out) {
    ...
    if (str.size() >= 1 &&
        str[0] ==
            '-') // Reject negative values, unfortunately strtoul accepts these by default if they fit in the range
        return false;
    char* endp          = nullptr;
...
bool ParseUInt64(const std::string& str, uint64_t* out) {
    ...
    if (str.size() >= 1 &&
        str[0] ==
            '-') // Reject negative values, unfortunately strtoull accepts these by default if they fit in the range
        return false;
    char* endp               = nullptr;
