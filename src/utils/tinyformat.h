...
#define TINYFORMAT_DEFINE_FORMATVALUE_CHAR(charType)                                                 \
    inline void formatValue(                                                                         \
        std::ostream& out, const char* /*fmtBegin*/, const char* fmtEnd, int /**/, charType value) { \
        switch (*(fmtEnd - 1)) {                                                                     \
        case 'u':                                                                                    \
        case 'd':                                                                                    \
        case 'i':                                                                                    \
...
inline void formatImpl(std::ostream& out, const char* fmt, const detail::FormatArg* formatters, int numFormatters) {
    ...
    for (int argIndex = 0; argIndex < numFormatters; ++argIndex) {
        ...
        int ntrunc            = -1;
        const char* fmtEnd =
            streamStateFromFormat(out, spacePadPositive, ntrunc, fmt, formatters, argIndex, numFormatters);
        if (argIndex >= numFormatters) {
...
#define TINYFORMAT_MAKE_FORMATLIST_CONSTRUCTOR(n)                                       \
    ...
    explicit FormatListN(TINYFORMAT_VARARGS(n)) : FormatList(&m_formatterStore[0], n) { \
        assert(n == N);                                                                 \
