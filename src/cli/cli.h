...
public:
    Cli(std::unique_ptr<Menu>&& _rootMenu,
        ...
        : rootMenu(std::move(_rootMenu)), exitAction(_exitAction) {}
    ...
    static void Register(std::ostream& o) {
        cout().Register(o);
    }
    static void UnRegister(std::ostream& o) {
        cout().UnRegister(o);
    }
...
public:
    ...
    CmdHandler(std::weak_ptr<Command> c, std::weak_ptr<CmdVec> v) : descriptor(std::make_shared<Descriptor>(c, v)) {}
    void Enable() {
        if (descriptor)
            descriptor->Enable();
    }
...
private:
    struct Descriptor {
        Descriptor() {}
        Descriptor(const std::weak_ptr<Command>& c, const std::weak_ptr<CmdVec>& v) : cmd(c), cmds(v) {}
        void Enable() {
            if (auto c = cmd.lock())
                c->Enable();
        }
...
public:
    ...
    template <typename F>
    [[deprecated("Use the method Insert instead")]] void Add(const std::string& name,
                                                             F f,
                                                             const std::string& help = "") {
    ...
    bool IsCmdAvailable(std::string& name) {
        for (auto& cmd : *cmds) {
            if ((cmd->Name() == name) && cmd->IsEnabled()) {
                return true;
            }
        }
        return false;
...
struct Select<F, P, Args...> {
    ...
    static void Exec(const F& f, InputIt first, InputIt last) {
        ...
        const P p = lexical_cast<typename std::decay<P>::type>(*first);
...
inline CliSession::CliSession(Cli& _cli, std::ostream& _out, std::size_t historySize)
    : cli(_cli), current(cli.RootMenu()), globalScopeMenu(std::make_unique<Menu>()), out(_out), history(historySize) {
    cli.Register(out);
    globalScopeMenu->Insert(
        "help", [this](std::ostream&) { Help(); }, "Help message");
    globalScopeMenu->Insert(
        "exit", [this](std::ostream&) { Exit(); }, "Exit");
...
inline void CliSession::Feed(const std::string& cmd) {
    ...
    auto cmd_name = strs[0];
    if (!current->IsCmdAvailable(cmd_name) && !globalScopeMenu->IsCmdAvailable(cmd_name)) {
        out << "Unknown command: " << cmd_name << std::endl;
        return;
    }
    ...
    if (!found)              // error msg if not found
        out << "Need more argument, see the command help: " << cmd_name << std::endl;
...
template <typename F, typename R, typename... Args>
CmdHandler Menu::Insert(const std::string& name,
                        const std::string& help,
                        const std::vector<std::string>& parDesc,
                        F& f,
                        R (F::*)(std::ostream& out, Args...) const) {
